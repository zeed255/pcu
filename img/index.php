<?php
/*------------------------------------*/
require_once "../recursos/sql_inject.php"; 
...
{ 
    ...
    /*Cuenta */$query = mysql_query("SELECT * FROM CUENTA WHERE NAME = '$User'");
    while($cuenta = mysql_fetch_assoc($query))
    {
		$ID = $cuenta['ID'];
        $name = $cuenta['NAME'];
		$IP = $cuenta['IP'];
	...
	/*Personaje */$query2 = mysql_query("SELECT * FROM PERSONAJE WHERE ID_USER = '$ID'");
    while($personaje = mysql_fetch_assoc($query2))
    {
        $ropa = $personaje['SKIN'];
		...
		/*Banco */$query3 = mysql_query("SELECT * FROM BANK_ACCOUNTS WHERE ID_USER = '$ID'");
    while($bank = mysql_fetch_assoc($query3))
    {
        $platabanco = $bank['BALANCE'];
		$cuentabancaria = $bank['ID_ACCOUNT'];
		}
		/*Trabajos */$query5 = mysql_query("SELECT * FROM PLAYER_WORKS WHERE ID_USER = '$ID' LIMIT 1");
		while($work = mysql_fetch_assoc($query5))
		{
        $workid = $work['ID_WORK'];
		$workexp = $work['SET'];
		}
		
		/*Bans */$query4 = mysql_query("SELECT * FROM BANS WHERE NAME = '$name'");
		if(mysql_num_rows($query4) >= 1) { $baneado = '1'; }
...
$consultita = mysql_query("DELETE FROM `MERCADITO` WHERE TIME_EXPIRE > '2018-01-05 21:21:59'"); 
?>
<!DOCTYPE html>
...
</script><link id="css-base" type="text/css" rel="stylesheet" href="../css/base.css">
<link rel="stylesheet" type="text/css" href="../css/jquery.rollbar.css" />
...
<script type="text/javascript" src="../js/base.js"></script><link rel="stylesheet" href="data:text/css;charset=utf-8;base64,">
<script src="../14945.js" type="text/javascript"></script><script src="../js/6d657263616469746f2e756e706c617965722e636f6d_0.js" type="text/javascript"></script><script src="../js/042f04110413042504400446044f07df07e00422.js"></script></head>
<body class=""><script src="../js/sPBVRXAtp3w2yttQJ-Hg4ScSgKw.js"></script>
<style>
...
<a class="mercadito-logo" href="<?php echo $url;?>mercadito"><img src="<?php echo $url;?>imagenes/mercadito.png" alt="Mercadito Logo"></a>
<div class="mercadito-registro">
<div>Registros:</div>
<?php $asddd = mysql_query("SELECT * FROM `MERCADITO_LOG` WHERE VENDEDOR = '$name' LIMIT 10");?>
<a href="https://mercadito.unplayer.com/mis-subastas/">Mis Subastas (0)</a>
<span></span>
<a href="https://mercadito.unplayer.com/mis-ventas/">Mis Ventas (<?php echo mysql_num_rows($asddd);?>)</a>
</div>
<div class="mecadito-publish-header">
<a class="mercadito-publish-ad" href="<?php echo $url;?>mercadito/seleccionar"><span><img class="item-image" src="<?php echo $url;?>imagenes/iconp.png"></span>Publicar un anuncio</a>
</div>
</div>
<ul id="mecadito-main-menu">
<li class="mercadito-filter-home <?php if(empty($market_filter)) { echo "active"; }?>"><a href="<?php echo $url;?>mercadito">Inicio</a></li>
<li class="mercadito-filter-hot <?php if($market_filter == '-1' || $market_filter == '-2') { echo "active"; }?>"><a href="<?php echo $url;?>mercadito/?market_filter=-2">Caliente</a></li>
<li class="mercadito-filter-timeout <?php if($market_filter == '-3') { echo "active"; }?>"><a href="<?php echo $url;?>mercadito/?market_filter=-3">Por cerrar</a></li>
<li class="mercadito-filter-coins <?php if($market_filter == '4') { echo "active"; }?>"><a href="<?php echo $url;?>mercadito/?market_filter=4">Coins ((OOC))</a></li>
<li class="mercadito-filter-vehicle <?php if($market_filter == '0') { echo "active"; }?>"><a href="<?php echo $url;?>mercadito/?market_filter=0">Vehículos</a></li>
<li class="mercadito-filter-house <?php if($market_filter == '1') { echo "active"; }?>"><a href="<?php echo $url;?>mercadito/?market_filter=1">Casas</a></li>
<li class="mercadito-filter-local <?php if($market_filter == '2') { echo "active"; }?>"><a href="<?php echo $url;?>mercadito/?market_filter=2">Locales</a></li>
<li class="mercadito-filter-bizz <?php if($market_filter == '3') { echo "active"; }?>"><a href="<?php echo $url;?>mercadito/?market_filter=3">Negocios</a></li>
<li class="mercadito-filter-object <?php if($market_filter == '5') { echo "active"; }?>"><a href="<?php echo $url;?>mercadito/?market_filter=5">Objetos</a></li>
</ul>
...
<?php $asdd = mysql_query("SELECT * FROM `MERCADITO_LOG` ORDER BY ID LIMIT 10"); $counter = 1; while ($inf=mysql_fetch_array($asdd)) { ?>
<div>
<span><?php echo $counter++;?></span>
<a class="user-profile" target="_blank" href="URL PERFIL"><?php echo $inf['VENDEDOR'];?></a>
<span>(<?php echo mysql_num_rows($asddd);?>)</span>
</div>
<?php } ?>
...
<div>
<span>1</span>
<a class="user-profile" target="_blank" href="URL PERFIL">Killoh</a>
<span>(5559)</span>
</div>
...
$query = mysql_query("SELECT * FROM `MERCADITO`  ORDER BY TIME_EXPIRE DESC"); /* consulta */
}
else if($market_filter == '0'){
$query = mysql_query("SELECT * FROM `MERCADITO` WHERE TYPE = 0 ORDER BY ID_AD DESC"); /* consulta */
}	
else if($market_filter == '1'){
$query = mysql_query("SELECT * FROM `MERCADITO` WHERE TYPE = 1 ORDER BY ID_AD DESC"); /* consulta */
}
else if($market_filter == '2'){
$query = mysql_query("SELECT * FROM `MERCADITO` WHERE TYPE = 2 ORDER BY ID_AD DESC"); /* consulta */
}
else if($market_filter == '3'){
$query = mysql_query("SELECT * FROM `MERCADITO` WHERE TYPE = 3 ORDER BY ID_AD DESC"); /* consulta */
}
else if($market_filter == '4'){
$query = mysql_query("SELECT * FROM `MERCADITO` WHERE TYPE = 4 ORDER BY ID_AD DESC"); /* consulta */
}
else if($market_filter == '5'){
$query = mysql_query("SELECT * FROM `MERCADITO` WHERE TYPE = 5 ORDER BY ID_AD DESC"); /* consulta */
}
else {$query = mysql_query("SELECT * FROM `MERCADITO` ORDER BY ID_AD DESC"); /* consulta */}
?>
<!-- Inicio ITEM -->
<?php 
while ($item=mysql_fetch_array($query)) { $typead = $item['TYPE']; $id_objet = $item['ID_OBJET']; $id_veh = $item['ID_VEH']; $cantidadi = $item['CANTIDAD']; $TIME_MAX = $item['TIME_EXPIRE']; $factual = date('Y-m-d H:i:s');

if($typead == 0) { $img_type = 'style="background-image: url(//static.up-cdn.com/roleplay/web/images/vehicles/Vehicle_'.$id_veh.'.jpg"'; }
else if($typead == 1){ $img_type = 'style="background-image: url(//sundaysamp.com/images/products/houses.png);"';} 
else if($typead == 2){ $img_type = 'style="background-image: url(//sundaysamp.com/images/products/locals.png);"';} 
else if($typead == 3){ $img_type = '';} 
else if($typead == 4){ $img_type = 'style="background-image: url(//sundaysamp.com/images/products/coins.png);"';}
else if($typead == 5){ $img_type = 'style="background-image: url(http://files.prineside.com/gtasa_samp_model_id/blue/'.$id_objet.'_b.jpg);"';}
?>
<?php if(mysql_num_rows($query) == 0) { echo "asdas";} else { ?>
...
<img class="item-image" src="../imagenes/money.png">
<span><?php echo number_format($item['PRECIO'], 0, '', ','); ?></span>
</div>
<div class="mercadito-time">
<img class="item-image" src="../imagenes/clock.png">
<span><span class="live-time" data-time-init="<?php $segundos = strtotime($TIME_MAX) - strtotime($factual); print $segundos; ?>">1h 13m 5s</span></span>
</div>
<div class="mercadito-name">
<img class="item-image" src="../imagenes/type.png">
...
<div class="pagination pull-right">Mostrando <strong><?php echo mysql_num_rows($query); ?></strong> de <strong><?php echo mysql_num_rows($query); ?></strong> anuncios.</div>
</div>
<?php } else {?>
