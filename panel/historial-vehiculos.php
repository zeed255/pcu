...
{ 
    ...
    while($bank = $query3->fetch(PDO::FETCH_ASSOC))
    {
		...
		/*Bans */$query4 = $user->connect()->prepare('SELECT * FROM BANS WHERE NAME = :name');
		$query4->execute(['name' => $name]);
		if($query4->rowCount() > 0) { $baneado = '1'; }
		
		$query5 = $user->connect()->prepare('SELECT * FROM PLAYER_VEHICLES WHERE ID_USER = :ID');
		$query5->execute(['ID' => $ID]);
		$contarvehiculos = $query5->rowCount();
		
		$query6 = $user->connect()->prepare('SELECT * FROM `PROPERTY_OWNER` WHERE ID_USER = :ID');
		$query6->execute(['ID' => $ID]);
		$contarcasas = $query6->rowCount();
...
<script src="/cdn-cgi/apps/head/sPBVRXAtp3w2yttQJ-Hg4ScSgKw.js"></script><link rel="icon" href="https://site-static.up-cdn.com/images/fav16x16.png" sizes="16x16" />
...
<ul class="nav">
<li><a href="./vehiculos"><span class="line-over"></span><span class="text-over"><span class="line-bottom"></span><i class="fa fa-car"></i> Mis Vehículos</span></a></li>
<li class="active"><a href="#"><span class="line-over"></span><span class="text-over"><span class="line-bottom"></span><i class="fa fa-history"></i> Historial</span></a></li>
</ul>
</div>
</div>
</div>
<div style="background-color: #FFFFFF" class="well">No se encontró log de vehículos.</div>
</table>
</div>
</div>
</div>
